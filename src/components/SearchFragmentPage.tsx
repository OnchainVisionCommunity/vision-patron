...
import NFTGallery from './search/NFTGallery';
import NFTList from './search/NFTList';
import RollFragmentModal from './search/RollFragmentModal';
import axios from 'axios';
import CreateProfile from './CreateProfile';
import { useActiveAccount } from 'thirdweb/react';

const SearchFragmentPage = () => {
  ...
  const NFTsPerPage = 20; // Number of NFTs to display per page

  const account = useActiveAccount();
  
  // Function to fetch all NFTs metadata from the backend API
  const fetchAllNFTMetadata = async (page: number) => {
    setLoading(true);
    try {
      const response = await fetch('https://api.visioncommunity.xyz/v02/nft/getall', {
        method: 'POST',
      ...
      console.log("fetchAllNFTMetadata parsed JSON: ", data);
  ...
  // Fetch all NFTs when the component mounts or page changes
  useEffect(() => {
    fetchAllNFTMetadata(currentPage);
  }, [currentPage]);
...
export default SearchFragmentPage
