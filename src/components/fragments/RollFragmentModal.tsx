...
const handleRollFragment = async () => {
  ...
  return (
    <Modal
      open={open}
      onClose={handleClose} 
      className={`modal-animation ${animationClass}`}
    ...
    <FontAwesomeIcon icon={faTrophy} style={{ color: '#0070f3' }} />
        ...
        >
          <Box sx={{ display: 'flex', flexDirection: 'column' }}>
            <Typography variant="h6" sx={{ fontWeight: 'bold', color: '#000' }}>
              You can win up to {formatPrize(displayMaxPrize)} $VISION
            </Typography>
          </Box>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', mt: 2 }}>
          <Button
            ...
            sx={{ flex: 1, mr: 1 }}
            onClick={handleRollFragment}
            disabled={nft.rolled || rolled || loading} // Disable button after roll
          >
            {rolled || nft.rolled ? 'Fragment Rolled' : loading ? 'Processing...' : 'Roll Now'}
          </Button>
