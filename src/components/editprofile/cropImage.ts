  ...
  if (canvas.width > maxWidth) {
    ...
    return new Promise<string>((resolve, reject) => {
      resizedCanvas.toBlob((blob) => {
        ...
        reader.onloadend = () => {
          resolve(reader.result as string); // Return the Base64 string of the resized image
        };
      }, "image/jpeg");
    });
  }

  // Return the cropped image as Base64 string
  return new Promise<string>((resolve, reject) => {
    canvas.toBlob((blob) => {
      if (!blob) {
        reject("Canvas is empty");
      ...
      reader.readAsDataURL(blob);
      reader.onloadend = () => {
        resolve(reader.result as string); // Return the Base64 string of the cropped image
      };
    }, "image/jpeg");
  });
}
