...
{/* Display Token Name & Symbol below the input if token is found 
{customToken && (
  <div className="tokenfound">
    {customToken.name} (${customToken.symbol})
          ...
          {showAdvancedOptions && (
            <div className="mt-4 p-4 advdiv">
              ...
              <div className="smalltextadvtip">The number of tokens added to the pool determines both the amount of energy you gain and the reputation earned by the community. Also is key factor to determine max prizes in lotteries and games.</div>
            </div>
          )}
          <hr className="sep marginhrtopbottom" />
          
{/* Show message if no token is found */}
{!customToken && currency === 'custom' && (
  <div className="nofounerror">No custom token found. Please enter a valid contract address.</div>
)}
...
<button
  ...
  disabled={!account?.address || !amount || !recipient || isTransactionInProgress} // Disable button during the transaction
>
  Patron Onchain
</button>
